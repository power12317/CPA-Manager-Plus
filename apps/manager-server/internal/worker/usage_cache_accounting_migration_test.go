...
func TestUsageCacheAccountingWorkerDrainsCompletionAndFencesLateStart(t *testing.T) {
	...
	w := NewUsageCacheAccountingMigrationWorker(st, func() { close(started); <-finish })
	...
	defer cancel()
	if err := w.StopAndWait(short); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("close must drain completion: %v", err)
	}
	close(finish)
	if err := w.StopAndWait(context.Background()); err != nil {
		t.Fatal(err)
	}
	stopped := NewUsageCacheAccountingMigrationWorker(st, func() { t.Error("late worker started") })
	if err := stopped.StopAndWait(context.Background()); err != nil {
		t.Fatal(err)
	}
	stopped.Start(context.Background())
	if stopped.done != nil {
		t.Fatal("stop must fence delayed start")
	}
}
