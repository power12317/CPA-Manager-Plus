...
import { UsageCoverageWarning, USAGE_COVERAGE_WARNING_DISMISSED_KEY } from './UsageCoverageWarning';
...
describe('UsageCoverageWarning', () => {
  ...
  it('keeps dismissal local to one instance while switching without a reload', () => {
    const coverage = {
      scope: 'time_range',
      mode: 'mixed' as const,
      raw_complete: false,
    ...
    const page = (scopeKey: string) => (
      <UsageCoverageWarning t={t} coverage={coverage} scopeKey={scopeKey} />
    );
    let renderer!: ReactTestRenderer;
    act(() => {
      renderer = create(page('manager/api/instances/a'));
    });
    act(() => {
    ...
    act(() => {
      renderer.update(page('manager/api/instances/b'));
    });
    expect(renderer.toJSON()).not.toBeNull();
    act(() => {
      renderer.update(page('manager'));
    });
    expect(renderer.toJSON()).not.toBeNull();
    act(() => {
      renderer.update(page('manager/api/instances/a'));
    });
    expect(renderer.toJSON()).toBeNull();
    act(() => {
      renderer.unmount();
