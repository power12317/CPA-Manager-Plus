...
export function UsageCoverageWarning({
  ...
  const scopedStorageKey =
    storageKey ??
    (scopeKey
      ? `${USAGE_COVERAGE_WARNING_DISMISSED_KEY}:${scopeKey}`
      : USAGE_COVERAGE_WARNING_DISMISSED_KEY);
  return (
    <UsageCoverageWarningContent key={scopedStorageKey} {...props} storageKey={scopedStorageKey} />
  );
}

function UsageCoverageWarningContent({
  coverage,
  t,
  dismissible = true,
