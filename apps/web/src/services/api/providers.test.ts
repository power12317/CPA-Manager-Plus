...
describe('providersApi auth-index preservation', () => {
  it('cancels a Meta provider save after the selected instance changes', async () => {
    ...
    const saving = providersApi.createMetaConfig({ apiKey: 'meta-inference-key' });
    const outcome = Promise.allSettled([saving]);
    await Promise.resolve();
    mocks.scopeController.abort();
    resolveRead({ 'meta-api-key': [] });
